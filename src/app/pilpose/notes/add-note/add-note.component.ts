...
export class AddNoteComponent implements OnInit {
  ...
  constructor(private router: Router,public toast: ToastrService,
    ...
    public formBuilder: UntypedFormBuilder,private noteService: AddNoteService,private tacheService: TacheService,
  ...
  onSubmit() {
    ...
    this.noteService.addOrUpdateNote(note).then(res => {
      
      
      this.toast.success(this.translate.instant('Note de Frais ajoutée avec succés'), '', Constants.toastOptions);
      ...
      this.toast.error(this.translate.instant('Erreur lors de la création d une Note de frais'), '', Constants.toastOptions);
  ...
  onUpload(id : number): void {
    ...
    this.http.post('http://localhost:8888/note/recu/'+id, formData)
      .subscribe(
        () => {
